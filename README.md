...
- **桌面风扇挂件**：透明悬浮小风扇（经典 3D 渲染），扇叶按档位旋转、摆头可见，是日常主交互入口
- **挂件交互**：点击扇头开关机、滚轮/圆点换档、悬停 + ←/→ 手动摆头、拖拽移动、右键菜单
- **氛围动效**：开机时风扇周围升起透明小泡（数量/升速随风速）；风声模拟按档位播放（可静音）；外轮廓边缘羽化柔和融入桌面
...
- **偏好记忆**：档位、摇头、角度、模式、风声静音、各连接参数自动保存
...
│   │   └── ble.py          蓝牙 BLE 设备
│   └── gui/                GUI：主窗 + 挂件 + 风声播放
│       └── audio.py        风声播放（winsound 零依赖封装）
├── assets/audio/           三档风声样本（离线合成，无版权问题）
├── firmware/esp32_fan/     ESP32 参考固件（Arduino）
├── installer/              Inno Setup 安装包脚本
├── launcher.py             PyInstaller 打包入口
├── tests/                  单元测试
├── tools/
│   └── gen_wind_sound.py   风声样本离线生成器（粉红噪声 + 阵风调制）
├── docs/                   产品文档 + 硬件搭建指南
├── CHANGELOG.md            版本变更日志
